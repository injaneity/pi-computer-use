...
mod native {
    ...
    fn element_to_json(store: &mut RefStore, element: &IUIAutomationElement) -> Option<Value> {
        ...
        let value = value_pattern
            ...
            .unwrap_or_default();
        let can_set_value = value_pattern
            .as_ref()
            .and_then(|pattern| unsafe { pattern.CurrentIsReadOnly().ok() })
            .map(|read_only| !read_only.as_bool())
            .unwrap_or(false);
        let can_invoke = unsafe { element.GetCurrentPattern(UIA_InvokePatternId).is_ok() };
        let can_scroll = unsafe { element.GetCurrentPattern(UIA_ScrollPatternId).is_ok() };
