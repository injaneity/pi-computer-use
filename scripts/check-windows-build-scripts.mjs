 ...
 *
 * Verifies:
 *   1. Path constants stay aligned with src/platform/windows.ts
...
// while still catching drift between the scripts and src/platform/windows.ts.
// ---------------------------------------------------------------------------
...
// Expected by src/platform/windows.ts → WINDOWS_HELPER_STABLE_PATH:
...
tap(
  ...
  "setup-helper.mjs dest matches WINDOWS_HELPER_STABLE_PATH in src/platform/windows.ts",
);
...
console.log(`\n${LABEL} build-native.mjs --platform darwin on Linux`);

{
  const result = await runScript("build-native.mjs", ["--platform", "darwin"]);
  tap(result.code, 1, "--platform darwin exits 1 on Linux (no xcrun)");
...

{
  ...
  if (isWin32()) {
    ...
    tap(result.code, 1, "no-args exits 1 on non-Windows");
...

{
  ...
  if (isWin32()) {
    ...
    tap(result.code, 0, "--postinstall exits 0 on non-Windows");
