---
title: 'Architecture'
description: 'How immutable UI state, resource scheduling, observations, and checked actions fit together.'
---
...
The bridge resolves model intent; the backend/helper owns grounding, preflight, delivery, and evidence. Accessibility capabilities choose an initial strategy but are not treated as proof that the intended result occurred. Editable-region clicks establish foreground focus for following unscoped keyboard steps in the same transaction. Raw coordinates are tied to the image-bearing state that produced them. A helper result reports `worked`, `didnt`, or `unknown`, including evidence and shallow root changes where available. The [macOS implementation reference](../reference/platforms/macos/actions.mdx) documents that backend's exact Accessibility, web-control, and synthesized-event paths.

With `headless: true`, the background boundary is strict: Pi must never activate or raise an application, change the user's focused window, move the global cursor, post synthesized input, or display the agent cursor. With `headless: false` (the default), credible semantic activation may begin in the background, editable clicks preserve the focus they establish for following unscoped keyboard input, and keyboard input with a checked `didnt` result may retry in the foreground because the first attempt proved side-effect-free. Focus-preserving native keyboard requests do not re-focus the outer window between semantic activation and event delivery. Ambiguous pointer outcomes are never replayed. On macOS, `cursor_overlay` schedules a visual, noninteractive overlay separately from action delivery; it is currently main-display-only ([macOS implementation reference](../reference/platforms/macos/actions.mdx#agent-cursor-overlay)).
...
The native transports accept multiple in-flight requests and correlate responses by request id. Both platforms keep eight immutable native look records and serialize global synthesized input. Platform-specific locking, event delivery, and occlusion behavior live in the [macOS implementation reference](../reference/platforms/macos/implementation.mdx) and [Windows bridge reference](../reference/platforms/windows.mdx) rather than being duplicated here.
...
OS-specific mechanisms can differ—Accessibility, Core Graphics, ScreenCaptureKit, Vision, and AppKit on macOS; UIA and Windows capture/input APIs on Windows—but state ownership, bounds, progressive disclosure, transaction boundaries, and honest outcomes may not. The [macOS implementation reference](../reference/platforms/macos/implementation.mdx) records the current API choices, delivery policies, evidence boundaries, and known limits so this explanation does not duplicate platform claims.
